...
public class CallbackConnection {
    ...
    void handleSessionFailure(Throwable error) {
        ...
        System.out.println("VIJAY - MQTT Reconnecting?" + error);
        if( !disconnected && (mqtt.reconnectAttemptsMax<0 || reconnects < mqtt.reconnectAttemptsMax ) ) {
            System.out.println("VIJAY - MQTT -- yes, reconnecting");
            ...
            if(t!=null) {
                t.stop(new Runnable() {
                    public void run() {
                        System.out.println("VIJAY - MQTT Trying to stop transport");
                        listener.onDisconnected();
                        try {
                            // And reconnect.
                            createTransport(new LoginHandler(new Callback<Void>() {
                                public void onSuccess(Void value) {
                                    System.out.println("VIJAY - MQTT reconnect succeeded");
                                ...
                                public void onFailure(Throwable value) {
                                    System.out.println("VIJAY - MQTT reconnect failed");
    ...
    void reconnect(final Callback<Transport> onConnect) {
        ...
        reconnects += 1;
        System.out.println("VIJAY - MQTT reconnect called " + reconnects);
        queue.executeAfter(reconnectDelay, TimeUnit.MILLISECONDS, new Runnable() {
            public void run() {
                System.out.println("VIJAY-MQTT actual reconnect called " + disconnected);
                if(disconnected) {
    ...
    void createTransport(final Callback<Transport> onConnect) throws Exception {
        ...
        transport.setTransportListener(new DefaultTransportListener(){
            public void onTransportConnected() {
                System.out.println("VIJAY - MQTT transport connected:" + disconnected);
                if(disconnected) {
            ...
            public void onTransportFailure(final IOException error) {
                System.out.println("VIJAY - MQTT transport failed");
                onFailure(error);
            }
    ...
    class LoginHandler implements Callback<Transport> {
        ...
        private boolean tryReconnect() {
            System.out.println("VIJAY - MQTT -- tryReconnect " + initialConnect);
            if(initialConnect) {
    ...
    private void processFrame(MQTTFrame frame) {
        try {
            switch(frame.messageType()) {
                ...
                case PINGRESP.TYPE: {
                    System.out.println("VIJAY--received ping");
                    pingedAt = 0;
                    break;
                }
                default:
    ...
    private void handleFatalFailure(Throwable error) {
        System.out.println("VIJAY -- MQTT Fatal Failure");
        if( failure == null ) {
            failure = error;
