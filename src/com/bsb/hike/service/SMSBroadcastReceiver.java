...
public class SMSBroadcastReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (extras != null) {
			Log.d(getClass().getSimpleName(),
					"Received SMS message with extras " + extras.keySet());
			Object[] extra = (Object[]) extras.get("pdus");
			Log.d(getClass().getSimpleName(), "Extras length is "
					+ extra.length);
			for (int i = 0; i < extra.length; ++i) {
				...
				try {
					...
					Log.d(getClass().getSimpleName(),
							"Received SMS " + msg.toString());
					Intent smsReceivedIntent = new Intent(context,
							HikeService.class);
					smsReceivedIntent.putExtra(
							HikeConstants.Extras.SMS_MESSAGE, msg.toString());
					context.startService(smsReceivedIntent);

					this.abortBroadcast();
				} catch (JSONException e) {
					Log.e(getClass().getSimpleName(),
							"Invalid data for SMS message", e);
					return;
				}
	...
	private void writeToNativeSMSDb(Context context, JSONObject msg)
		...
		context.getContentResolver().insert(
				HikeConstants.SMSNative.INBOX_CONTENT_URI, values);
	}
}
