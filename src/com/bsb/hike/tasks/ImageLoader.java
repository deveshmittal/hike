			...
			try
			{
				while (true)
				{
					...
					if (!listItemQueue.listItemRefs.isEmpty())
					{
						...
						ImageViewerInfo tag = (ImageViewerInfo) listItemToLoad.imageView.getTag();
						...
						if (tag != null)
						{
							...
							{
								id = Utils.getDefaultAvatarServerName(tag.mappedId);
							}
							else
							{
								id = tag.mappedId;
							...
							{
								fileName = Utils.getProfileImageFileName(listItemToLoad.id);
							}
							...
							try
							{
								File tempFile = File.createTempFile(listItemToLoad.id, ".jpg", outputDir);
								...
								imageMap.put(listItemToLoad.id, new SoftReference<Bitmap>(bmp));

								if (id.equals(listItemToLoad.id))
								{
									BitmapDisplayer bmpDisplayer = new BitmapDisplayer(bmp, listItemToLoad.imageView);

									Activity a = (Activity) listItemToLoad.imageView.getContext();
