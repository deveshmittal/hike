...
public class HikeHTTPTask extends AsyncTask<HikeHttpRequest, Integer, Boolean>
	...
	protected void onPostExecute(Boolean result) {
		...
		if (result.booleanValue()) {
			...
			if (finishableEvent != null) {
				Toast toast = Toast.makeText((Activity) finishableEvent,
						errorStringId, duration);
				toast.show();
			}
		}
	}
	...
	protected Boolean doInBackground(HikeHttpRequest... requests) {
		...
		try {
			for (HikeHttpRequest hikeHttpRequest : requests) {
				Log.d("HikeHTTPTask", "About to perform request:"
						+ hikeHttpRequest.getPath());
				AccountUtils.performRequest(hikeHttpRequest, addToken);
				Log.d("HikeHTTPTask", "Finished performing request:"
						+ hikeHttpRequest.getPath());
			}
		} catch (Exception e) {
