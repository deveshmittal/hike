	...
	protected Boolean doInBackground(Void... params) {
		try {
			...
			if (TextUtils.isEmpty(result)) {
				...
				try {
					Editor editor = hikeService.getSharedPreferences(
							HikeMessengerApp.ACCOUNT_SETTINGS, 0).edit();

					JSONObject response = new JSONObject(result);

					/*
					 * Here we check the source where this app was downloaded
					 * from if it was download from the play store, the source
					 * won't be null and we store the update url as blank so
					 * that the user is redirected to the play store.
					 */
					String updateUrl = !TextUtils.isEmpty(hikeService
							.getPackageManager().getInstallerPackageName(
									hikeService.getPackageName())) ? ""
					...
					if (!TextUtils.isEmpty(criticalVersion)) {
						if (Utils
								.isUpdateRequired(criticalVersion, hikeService)) {
							Log.d(getClass().getSimpleName(), "Critical update");
							editor.putInt(
									HikeConstants.Extras.UPDATE_AVAILABLE,
									HikeConstants.CRITICAL_UPDATE);
							editor.putString(
