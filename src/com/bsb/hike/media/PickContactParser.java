...
public class PickContactParser {
	public static PhonebookContact onActivityResult(int resultCode,
			Intent data, Context context) {
	...
	/**
	 * This function queries contact table for data, It is preferable to call
	 * this function in non UI thread
	 * 
	 * @param contactId
	 * @param context
	 * @return {@link PhonebookContact} Or null if some exception occurs
	 */
	public static PhonebookContact getContactData(String contactId,
			Context context) {
		...
		mimeTypes.append(DatabaseUtils.sqlEscapeString(Event.CONTENT_ITEM_TYPE)
				+ ",");
		mimeTypes.append(DatabaseUtils
				.sqlEscapeString(StructuredName.CONTENT_ITEM_TYPE) + ")");
		...
		try {
			while (c.moveToNext()) {
				...
				if (Phone.CONTENT_ITEM_TYPE.equals(mimeType)) {
					...
					String type = Phone.getTypeLabel(context.getResources(),
					...
					String type = Email.getTypeLabel(context.getResources(),
							c.getInt(data2Idx), c.getString(data3Idx))
							.toString();
					...
					String type = StructuredPostal.getTypeLabel(
							context.getResources(), c.getInt(data2Idx),
							c.getString(data3Idx)).toString();
