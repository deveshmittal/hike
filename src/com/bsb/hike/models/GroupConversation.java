...
public class GroupConversation extends Conversation {
	...
	public GroupConversation(JSONObject jsonObject, Context context)
		...
		for (int i = 0; i < array.length(); i++) {
			...
			GroupParticipant groupParticipant = new GroupParticipant(
					new ContactInfo(contactNum, contactNum, contactName,
							contactNum, onHike), false, onDnd);
			Log.d(getClass().getSimpleName(),
					"Parsing JSON and adding contact to conversation: "
							+ contactNum);
	...
	public GroupParticipant getGroupParticipant(String msisdn) {
		return groupParticipantList.containsKey(msisdn) ? groupParticipantList
				.get(msisdn) : new GroupParticipant(new ContactInfo(msisdn,
	...
	public JSONObject serialize(String type) {
		...
		try {
			...
			if (type.equals(HikeConstants.MqttMessageTypes.GROUP_CHAT_JOIN)) {
				...
				for (Entry<String, GroupParticipant> participant : groupParticipantList
						.entrySet()) {
					JSONObject nameMsisdn = new JSONObject();
					nameMsisdn.put(HikeConstants.NAME, participant.getValue()
