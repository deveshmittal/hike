...
public class MessageMetadata {
	...
	public MessageMetadata(JSONObject metadata) throws JSONException {
		this.participantInfoState = metadata.has(HikeConstants.DND_USERS)
				|| metadata.has(HikeConstants.DND_NUMBERS) ? ParticipantInfoState.DND_USER
		...
		case DND_USER:
			this.dndNumbers = metadata.has(HikeConstants.DND_USERS) ? metadata
					.getJSONArray(HikeConstants.DND_USERS) : metadata
					.getJSONArray(HikeConstants.DND_NUMBERS);
			break;
		...
		case PARTICIPANT_LEFT:
			...
			this.showBIS = HikeConstants.MqttMessageTypes.BLOCK_INTERNATIONAL_SMS
					.equals(metadata.optString(HikeConstants.SUB_TYPE));
			break;

		case USER_JOIN:
		case USER_OPT_IN:
			this.msisdn = metadata.getJSONObject(HikeConstants.DATA).getString(
		...
		this.newUser = metadata.optString(HikeConstants.NEW_USER)
				.equals("true");
		this.dndMissedCallNumber = metadata
				.optString(HikeConstants.METADATA_DND);
		this.hikeFileList = getHikeFileListFromJSONArray(metadata
				.optJSONArray(HikeConstants.FILES));
		if (HikeConstants.LOCATION_CONTENT_TYPE.equals(metadata
				.optString(HikeConstants.CONTENT_TYPE))) {
		...
		if (metadata.has(StickerManager.STICKER_ID)) {
			this.sticker = new Sticker(
					metadata.optString(StickerManager.CATEGORY_ID),
					metadata.optString(StickerManager.STICKER_ID),
	...
	public Spannable getMessage(final Context context,
			final ConvMessage convMessage, boolean shouldSetClickListener) {
		String content = "tap here";
		StringBuilder formatArg = new StringBuilder();
		final StringBuilder dndMsisdn = new StringBuilder();
		if (dndNumbers != null) {
			try {
				Map<String, GroupParticipant> participantList = ((GroupConversation) convMessage
						.getConversation()).getGroupParticipantList();
				for (int i = 0; i < dndNumbers.length(); i++) {
					GroupParticipant dndParticipant = participantList
							.get(dndNumbers.getString(i));
					if (dndParticipant != null) {
						String separator = (i == (dndNumbers.length() - 2)) ? " and "
								: ((i < dndNumbers.length() - 2) ? ", " : "");
						formatArg.append(dndParticipant.getContactInfo()
								.getFirstName() + separator);
					}
					String msisdnSeparator = (i < (dndNumbers.length() - 1)) ? Build.MANUFACTURER
			...
			formatArg.append(Utils.getFirstName(convMessage.getConversation()
					.getLabel()));
			dndMsisdn.append(convMessage.getMsisdn());
		}
		String message = context.getString(!TextUtils
				.isEmpty(dndMissedCallNumber) ? R.string.dnd_message
				: !newUser ? R.string.friend_joined_hike_no_creds
						: R.string.friend_joined_hike_with_creds, formatArg,
				dndMissedCallNumber);
			...
			spannable.setSpan(
					new ClickableSpan() {
						...
						public void onClick(View blah) {
							Intent intent = !TextUtils
									.isEmpty(dndMissedCallNumber) ? new Intent(
									Intent.ACTION_VIEW) : new Intent(context,
									CreditsActivity.class);
							if (!TextUtils.isEmpty(dndMissedCallNumber)) {
								Utils.logEvent(context,
										HikeConstants.LogEvent.OPT_IN_TAP_HERE);
								intent = new Intent(Intent.ACTION_VIEW);
								intent.setData(Uri.parse("sms:" + dndMsisdn));
								intent.putExtra("sms_body", context.getString(
										R.string.dnd_invite_message,
										dndMissedCallNumber));
							}
