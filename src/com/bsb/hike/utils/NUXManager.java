...
public class NUXManager
{
	...
	public void parseNuxPacket(String json, Context context)
	{
			...
			if (root.optString(HikeConstants.SUB_TYPE).equals(NUXConstants.SUBTYPE_NEW))
			{
				...
				if (!TextUtils.isEmpty(root.optString(HikeConstants.DATA)))
				{
					...
					if (mmJsonObject != null)
					{
						...
						if (getNuxInviteFriendsPojo().isToggleSkipButton())
						{
							setCurrentState(NUXConstants.NUX_SKIPPED);
						}
			...
			try
			{
				...
				if (!TextUtils.isEmpty(task_details.toString()))
				{
					...
					if(incentive!=null)
					{
						...
						{
								incentiveAmount=incentive.optInt(NUXConstants.INCENTIVE_AMOUNT);
