...
public class HighlightView {
	...
	private void init() {
		...
		mResizeDrawable = resources
				.getDrawable(R.drawable.camera_crop_indicator);
		...
		if (!hasFocus()) {
			...
			if (mMode == ModifyMode.Grow) {
				if (mCircle) {
					...
					mResizeDrawable.setBounds(left - width, top - height, left
							+ width, top + height);
					mResizeDrawable.draw(canvas);

					mResizeDrawable.setBounds(right - width, top - height,
							right + width, top + height);
					mResizeDrawable.draw(canvas);

					mResizeDrawable.setBounds(left - width, bottom - height,
							left + width, bottom + height);
					mResizeDrawable.draw(canvas);

					mResizeDrawable.setBounds(right - width, bottom - height,
