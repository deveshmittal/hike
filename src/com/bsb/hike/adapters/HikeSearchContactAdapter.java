...
public class HikeSearchContactAdapter extends ArrayAdapter<Pair<AtomicBoolean, ContactInfo>> implements TextWatcher
{
	...
	public HikeSearchContactAdapter(Activity context, List<Pair<AtomicBoolean, ContactInfo>> contactList, EditText inputNumber, String groupId, boolean forwarding,
	...
	public View getView(int position, View convertView, ViewGroup parent)
	{
		...
		numberTextView.setText(contactInfo != null ? contactInfo.getMsisdn() : context.getString(R.string.tap_to_add));
		...
		if (contactInfo != null && contactInfo.getId().equals(contactInfo.getPhoneNum()))
		{
		...
		else
		{
			...
			{
				avatar.setImageDrawable(context.getResources().getDrawable(R.drawable.ic_avatar1_rounded));
			}
		}
	...
	private class ContactFilter extends Filter
	{
		...
		protected FilterResults performFiltering(CharSequence constraint)
		{
			...
			if (!TextUtils.isEmpty(textToBeFiltered) || !TextUtils.isEmpty(groupId))
			{
				...
				if (shouldShowExtraElement(textToBeFiltered))
				{
					...
					{
						filteredContacts.add(new Pair<AtomicBoolean, ContactInfo>(new AtomicBoolean(true), null));
					}
					else
					{
						filteredContacts.add(new Pair<AtomicBoolean, ContactInfo>(new AtomicBoolean(), null));
					}
				}
	...
	private String getNumber(String textInEditText)
	{
		...
		return !textInEditText.contains(HikeConstants.GROUP_PARTICIPANT_SEPARATOR) ? textInEditText : textInEditText.substring(indexTextToBeFiltered);
	}
