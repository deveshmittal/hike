...
import com.bsb.im.ui.wizard.Account;
 ...
 * 
 */
public class CreateAccount extends Activity implements OnTouchListener {
	...
	private boolean createAccount(String username, String password) {
		...
		// wyk
		/*
		 * ProxyInfo pi = getRegisteredProxy(); if (pi != null) {
		 * connectionConfiguration = new
		 * ConnectionConfiguration(getXMPPServer(), getXMPPPort(), pi); } else {
		 * connectionConfiguration = new
		 * ConnectionConfiguration(getXMPPServer(), getXMPPPort()); }
		 */
		connectionConfiguration = new ConnectionConfiguration("125.21.240.144",
				5222, "boco.jp");// (getXMPPServer(), getXMPPPort());
		connectionConfiguration.setTruststoreType("bks");
		connectionConfiguration.setSASLAuthenticationEnabled(false);
		// wyk
		/*
		 * if (getRegisteredXMPPTLSUse())
		 * connectionConfiguration.setSecurityMode
		 * (ConnectionConfiguration.SecurityMode.required);
		 */

		// ContactMap cm = new ContactMap(this);
		Map<String, String> mapInfo = new HashMap<String, String>();
		// String phone = "13693298112";//cm.getPhoneNumber();
		// String addressbook = cm.getContact();
		// if(addressbook.equals(""))
		// {
		// addressbook = "wyk:16591112578";
		// }
		mapInfo.put("phone", phone);
		// mapInfo.put("address-book", addressbook);
		...
		try {
			...
			accountManager.createAccount(username, password, mapInfo);
			Toast toast = Toast
					.makeText(
							getApplicationContext(),
							String.format(
									getString(R.string.create_account_successfull_after),
									username), NOTIFICATION_DURATION);
			toast.show();
		} catch (Exception e) {
	...
	private void createErrorDialog(String errMsg) {
		...
		builder.setTitle(R.string.create_account_err_dialog_title)
				.setMessage(errMsg).setCancelable(false)
				.setIcon(android.R.drawable.ic_dialog_alert);
		builder.setNeutralButton(R.string.create_account_close_dialog_button,
	...
	private ProxyInfo getRegisteredProxy() {
		if (getRegisteredProxyUse()) {
			ProxyInfo proxyInfo = new ProxyInfo(getRegisteredProxyType(),
					getRegisteredProxyServer(), getRegisteredProxyPort(),
					getRegisteredProxyUsername(), getRegisteredProxyPassword());
			return proxyInfo;
		}
		return null;
	}
	...
	private int getRegisteredProxyPort() {
		return Integer.parseInt(mSettings.getString(
				BeemApplication.PROXY_PORT_KEY, DEFAULT_STRING_VALUE));
	}
	...
	private ProxyInfo.ProxyType getRegisteredProxyType() {
		...
		if (mSettings.getBoolean(BeemApplication.PROXY_USE_KEY, false)) {
			String type = mSettings.getString(BeemApplication.PROXY_TYPE_KEY,
					"none");
			if ("HTTP".equals(type))
	...
	private boolean getRegisteredProxyUse() {
		return mSettings.getBoolean(BeemApplication.PROXY_USE_KEY,
	...
	private String getRegisteredProxyUsername() {
		return mSettings.getString(BeemApplication.PROXY_USERNAME_KEY,
				DEFAULT_STRING_VALUE);
	}
	...
	private int getXMPPPort() {
		// wyk
		/*
		 * int port = DEFAULT_XMPP_PORT; if
		 * (mSettings.getBoolean("settings_key_specific_server", false)) port =
		 * Integer.parseInt(mSettings.getString("settings_key_xmpp_port",
		 * "5222")); return port;
		 */
		return 5222;
	}
	...
	private String getXMPPServer() {
		// wyk
		/*
		 * TextView xmppServerTextView = (TextView)
		 * findViewById(R.id.create_account_username); String xmppServer = "";
		 * if (mSettings.getBoolean("settings_key_specific_server", false))
		 * xmppServer = mSettings.getString("settings_key_xmpp_server", "");
		 * else xmppServer =
		 * StringUtils.parseServer(xmppServerTextView.getText().toString());
		 * return xmppServer;
		 */
		return "125.21.240.144";
	}
	...
	private boolean getRegisteredXMPPTLSUse() {
		return mSettings.getBoolean("settings_key_xmpp_tls_use",
				DEFAULT_BOOLEAN_VALUE);
	}
	...
	private boolean checkPasswords() {
		final String passwordFieldValue = ((EditText) findViewById(R.id.create_account_password))
				.getText().toString();
		...
		return passwordFieldValue.equals(passwordConfirmFielddValue);// &&
																		// !"".equals(passwordConfirmFielddValue);
	}
	...
	private boolean checkEmail() {
		 ...
		 * findViewById(R.id.create_account_username)).getText().toString();
		 * return
	 ...
	 */
	/*
	 * private void initCreateAccountButton() { backButton = (Button)
	 * findViewById(R.id.back_button); backButton.setOnClickListener(new
	 * View.OnClickListener() { public void onClick(View v) {
	 * 
	 * } }); Button createButton = (Button) findViewById(R.id.create_button);
	 * createButton.setOnClickListener(new View.OnClickListener() { public void
	 * onClick(View v) { String usernameFieldValue = ((EditText)
	 * findViewById(R.id.create_account_username)).getText().toString(); String
	 * username = StringUtils.parseName(usernameFieldValue); String
	 * passwordFieldValue = ((EditText)
	 * findViewById(R.id.create_account_password)).getText().toString();
	 * Log.d("username",usernameFieldValue);
	 * Log.d("passwordFieldValue",passwordFieldValue); if(passwordFieldValue ==
	 * null) { passwordFieldValue = ""; } if (!checkEmail())
	 * createErrorDialog(getString(R.string.create_account_err_username)); else
	 * if (!checkPasswords())
	 * createErrorDialog(getString(R.string.create_account_err_passwords)); else
	 * { if (createAccount(username, passwordFieldValue)) {
	 * SharedPreferences.Editor settingsEditor = mSettings.edit();
	 * settingsEditor.putString(BeemApplication.ACCOUNT_USERNAME_KEY,
	 * username);//usernameFieldValue);
	 * settingsEditor.putString(BeemApplication.ACCOUNT_PASSWORD_KEY,
	 * passwordFieldValue); settingsEditor.putBoolean("settings_key_gmail",
	 * false); settingsEditor.commit(); finish(); } } } }); }
	 */
	...
	public boolean onTouch(View v, MotionEvent arg1) {
		...
		if (arg1.getAction() == MotionEvent.ACTION_DOWN) {
			...
			if (v == backButton) {
				...
				String usernameFieldValue = ((EditText) findViewById(R.id.create_account_username))
						.getText().toString();
				phone = usernameFieldValue;
				// String username = StringUtils.parseName(usernameFieldValue);
				String passwordFieldValue = ((EditText) findViewById(R.id.create_account_password))
						.getText().toString();
				if (!checkEmail())
					...
					if (createAccount(usernameFieldValue + "@boco.jp",
							passwordFieldValue)) {
						SharedPreferences.Editor settingsEditor = mSettings.edit();
						settingsEditor.putString(
								BeemApplication.ACCOUNT_USERNAME_KEY,
								usernameFieldValue + "@boco.jp");
						settingsEditor.putString(
