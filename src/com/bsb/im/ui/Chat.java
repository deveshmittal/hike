...
import com.bsb.im.ui.dialogs.builders.ChatList;
 ...
 * 
 */
public class Chat extends Activity implements EditText.OnFocusChangeListener,
	...
	protected void onCreate(Bundle savedBundle) {
		...
		mInputField.setOnFocusChangeListener(this);
		...
		mSendButton.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent arg1) {
				...
				if (arg1.getAction() == MotionEvent.ACTION_DOWN) {
					v.setBackgroundResource(R.drawable.button_send_on);
				} else if (arg1.getAction() == MotionEvent.ACTION_UP) {
					v.setBackgroundResource(R.drawable.button_send);
					sendMessage();
				}
				return false;
			}
					...
					public void run() {
						if (msg.getType() == Message.MSG_TYPE_ERROR) {
							mListMessages.add(new MessageText(fromBareJid,
									...
									msg.getTimestamp(), msg.getTime()));
							mMessagesListAdapter.notifyDataSetChanged();
						} else if (msg.getBody() != null
								&& !msg.getBody().trim().equals("")) {
							MessageText lastMessage = null;
							...
							if (msg.getBody() != null)
								mListMessages.add(new MessageText(fromBareJid,
										...
										false, msg.getTimestamp(), msg
												.getTime()));
							mMessagesListAdapter.notifyDataSetChanged();

							// String mFrom =
							// StringUtils.parseBareAddress(msg.getFrom());
							// String mTo =
							// StringUtils.parseBareAddress(msg.getTo());
							// Date mDate = msg.getTimestamp();
							sendMessage(msg.getFrom(), "received",
									msg.getTime());
						} else {
							if (msg.getType() == Message.MSG_TYPE_CHAT) {

								if (msg.getReceived().equals("received")) {
									MessageText mt;
									for (int i = 0; i < mListMessages.size(); i++) {
										mt = (MessageText) mListMessages.get(i);
										if (mt.packetId.equals(msg.getTime())) {
											mt.setMessage(mt.getMessage()
													+ "   D");
											mListMessages.set(i, mt);
											mMessagesListAdapter
	...
	private class MessagesListAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			// add by song fei-------------------------
			LinearLayout ll = (LinearLayout) sv
					.findViewById(R.id.linearLayout1);
			if (msg.getName().equals("I") == true) {
				ll.setGravity(Gravity.RIGHT);
				msgName.setVisibility(View.INVISIBLE);
				msgText.setBackgroundResource(R.drawable.out_bubble_green);
			} else {
				ll.setGravity(Gravity.LEFT);
				msgName.setVisibility(View.VISIBLE);
				msgText.setBackgroundResource(R.drawable.in_bubble);
			}
			// end add by song fei-----------------------
	...
	private void sendMessage(String to, String s, String d) {
		Message msgToSend = new Message(to, Message.MSG_TYPE_CHAT, s, d);
	...
	private void sendMessage() {
		...
		if (!"".equals(inputContent)) {
			...
			/*
			 * MessageText lastMessage = null; if (mListMessages.size() != 0)
	...
	public void onFocusChange(View v, boolean hasFocus) {
		if (v == mInputField) {
			if (hasFocus) {
				...
				Log.d("aaaaaaaa", "222222222222222222");
			}
		}
