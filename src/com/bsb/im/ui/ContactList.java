...
import com.bsb.im.ui.dialogs.builders.Alias;
import com.bsb.im.ui.dialogs.builders.ChatList;
import com.bsb.im.ui.dialogs.builders.DeleteContact;
import com.bsb.im.ui.dialogs.builders.ResendSubscription;
import com.bsb.im.utils.BeemBroadcastReceiver;
import com.bsb.im.utils.SortedList;
...
public class ContactList extends Activity {
	...
	private class BeemRosterListener extends IBeemRosterListener.Stub {
		...
		public void onEntriesAdded(final List<String> addresses)
			...
			for (String newName : addresses) {
				...
				boolean visible = !hideDisconnected
				...
				if (visible) {
					for (String group : groups) {
						...
						List<Contact> contactByGroups = mContactOnGroup
								.get(group);
		...
		public void onEntriesDeleted(final List<String> addresses)
			...
			for (String cToDelete : addresses) {
				...
				for (Map.Entry<String, List<Contact>> entry : mContactOnGroup
						.entrySet()) {
		...
		private void updateCurrentList(String listName, final Contact contact) {
			...
			final boolean add = ((!hideDisconnected || Status
					.statusOnline(contact.getStatus())) && // must show and
		...
		private void cleanBannerGroup() throws RemoteException {
			...
			List<String> realGroups = mListGroup.subList(1,
					mListContact.size() - 1);
	...
	private static class BeemBanner extends BaseAdapter {
		 ...
		 */
		public BeemBanner(final LayoutInflater inflater,
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			...
			try {
				...
				if (mRoster != null) {
					...
					if (!mSettings.getBoolean(BeemApplication.HIDE_GROUPS_KEY,
							false))
						showGroups();
					else
						hideGroups();
			...
			for (Contact c : contacts) {
				...
				if (groups.isEmpty())
					...
					for (String currentGroup : groups) {
						List<Contact> contactsByGroups = mContactOnGroup
								.get(currentGroup);
