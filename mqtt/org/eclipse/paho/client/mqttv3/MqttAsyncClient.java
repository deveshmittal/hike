...
{ // DestinationProvider {
	 ...
	 * 
	 * <p>
	 * A client identifier <code>clientId</code> must be specified and be less that 23 characters. It must be unique across all clients connecting to the same server. The clientId
	 * is used by the server to store data related to the client, hence it is important that the clientId remain the same when connecting to a server if durable subscriptions or
	 ...
	 */
	public MqttAsyncClient(String serverURI, String clientId, int maxInflightMsgs) throws MqttException
	{
		this(serverURI, clientId, new MqttDefaultFilePersistence(), maxInflightMsgs);
	}
	...
	public MqttAsyncClient(String serverURI, String clientId, MqttClientPersistence persistence, int maxInflightMsgs) throws MqttException
	{
		...
		this.comms = new ClientComms(this, this.persistence, maxInflightMsgs);
	...
	public IMqttToken disconnect(long quiesceTimeout, Object userContext, IMqttActionListener callback) throws MqttException
	{
		...
		{
			// @TRACE 105=< exception
			Logger.d(TAG, "eception in disconnect reason : " + ex.getCause());
			throw ex;
		}
		// @TRACE 108=<
	...
	public void close() throws MqttException
	{
		...
		// @TRACE 113=<
		Logger.d(TAG, "mqttAsyncClient closing started");
		comms.close();
		// @TRACE 114=>
		Logger.d(TAG, "mqttAsyncClient closing completed");
