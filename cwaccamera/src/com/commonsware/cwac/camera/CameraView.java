...
//	    width = height = size;
	...
	{
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH && camera != null && !isDetectingFaces && camera.getParameters().getMaxNumDetectedFaces() > 0)
		{
	...
	private void setCameraPictureOrientation(Camera.Parameters params)
	{
		...
		Log.d("CameraView", "setCameraPictureOrientation");
		...
		if (getActivity().getRequestedOrientation() != ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED)
		{
			Log.d("CameraView", "setCameraPictureOrientation screenorientunspec");
			outputOrientation = getCameraPictureRotation(getActivity().getWindowManager().getDefaultDisplay().getOrientation());
		}
		else if (info.facing == Camera.CameraInfo.CAMERA_FACING_FRONT)
		{
			Log.d("CameraView", "setCameraPictureOrientationffc");
			outputOrientation = (360 - displayOrientation) % 360;
		}
		else
		{
			Log.d("CameraView", "setCameraPictureOrientationelse");
			outputOrientation = displayOrientation;
		}

		if (lastPictureOrientation != outputOrientation)
		{
			Log.d("CameraView", "setCameraPictureOrientationlastif");
		...
		public void onOrientationChanged(int orientation)
		{
			if (camera != null && orientation != ORIENTATION_UNKNOWN)
			{
				...
				if (newOutputOrientation != outputOrientation)
				{
					...
					Log.d("CameraView", "newOutputOrient: " + newOutputOrientation);
